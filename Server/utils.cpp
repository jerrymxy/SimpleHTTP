...
std::map<std::string, std::string> getRequestField(const std::vector<std::string>& split_str) {
    ...
    std::string url = split_str[0].substr(blank_location + 1, end - blank_location - 1);
    std::string http_version = split_str[0].substr(end + 1);
    ...
    if (request_type == "GET") {
        ...
        request_field["Body"] = split_str[split_str.size() - 1];
    }
